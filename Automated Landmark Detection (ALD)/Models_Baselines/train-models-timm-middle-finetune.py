...
CONFIG = {
    ...
    # 训练与日志
    "img_size": 512, #  Transformer/混合必须使用224 常规为512
    "batch_size": 480,
    "lr": 1e-3,
    "epochs": 500,
    "val_ratio": 0.15,
    "seed": 2024,
    "augment": True,
    "save_dir": r"D:\Codes\Skull_Landmarks_TL\Models-TIMM\runs\exp_timm_small_finetune",
    "num_workers": 4,            # Windows/Thonny 卡住就设 0
    "pin_memory": True,
    "tb_samples": 2,
    # 关键点
    "num_points": 12,
...
"backbones": [
    # middle
    "convnext_tiny", "resnet50", "regnety_016", "seresnext50_32x4d"
],
    ...
    "scheduler": {
        "use": True,
        "type": "cosine",     # 目前支持: "cosine" 或 None
        "eta_min": 1e-6       # 余弦末尾最小学习率
    },
}
...
def draw_points_on_image(img: Image.Image, points_xy_norm: np.ndarray, color=(255, 0, 0)):
    """在 PIL 图像上画归一化(0~1)坐标点，直接修改传入的 img。"""
    w, h = img.size
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            if self.geom_cfg.get("do_affine", True) and (self.geom_cfg.get("max_rotate_deg", 0) > 0
                ...
                # 对图应用仿射
                img = TF.affine(img, angle=angle, translate=(dx, dy), scale=scale, shear=[0.0, 0.0], center=center)
                # 对点应用同样仿射
...
class TimmBackboneRegressor(nn.Module):
    def __init__(self, out_dim=24, name="convnext_tiny", pretrained=True):
        super().__init__()
        # num_classes=0 -> 返回全局池化后的特征向量
        self.backbone = timm.create_model(name, pretrained=pretrained, num_classes=0, global_pool="avg")
        in_feats = self.backbone.num_features if hasattr(self.backbone, "num_features") else None
        if in_feats is None:
...
def eval_metrics_full(model, loader, device, img_size=512, pck_px=(2,4,8), pck_norm=(0.01, 0.02)):
    ...
    for imgs, targets, _ in loader:
        ...
        d_px = (pred_xy - gt_xy).pow(2).sum(dim=-1).sqrt()  # [B, num_points]
...
# =========================
# 封装：单个骨干的一次完整训练（含 Cosine LR + TB 记录 LR）
# =========================
def train_one_backbone(cfg, backbone_name: str):
    ...
    model = TimmBackboneRegressor(out_dim=out_dim, name=backbone_name, pretrained=cfg["pretrained"]).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=cfg["lr"], weight_decay=1e-4)
    criterion = nn.MSELoss()
    scaler = torch.cuda.amp.GradScaler(enabled=use_amp)

    # ===== 学习率调度器：CosineAnnealingLR（新增）=====
    scheduler = None
    if cfg["scheduler"].get("use") and cfg["scheduler"].get("type") == "cosine":
        from torch.optim.lr_scheduler import CosineAnnealingLR
        scheduler = CosineAnnealingLR(
            optimizer,
            T_max=cfg["epochs"],                 # 完整周期长度：整个训练
...
# tensorboard --logdir "Models-TIMM/runs/exp_timm_small_finetune/resnet18/logs"